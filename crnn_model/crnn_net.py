...
class ChineseCrnnNet:
    ...
    def _sequence_label(self, inputdata, name):
        ...
        with tf.variable_scope(name_or_scope=name):
            ...
            stack_lstm_layer = tf.nn.dropout(stack_lstm_layer, 0.5, name='sequence_drop_out_multi')
    ...
    def multi_gpu_train(self,
        ...
        """
        # define crnn network and optimizer
        global_step = tf.Variable(0, dtype=tf.int32, name='g_step_new', trainable=False)
        learning_rate = tf.train.exponential_decay(
            learning_rate=self.learning_rate,
            global_step=global_step,
            decay_steps=self.lr_decay_steps,
